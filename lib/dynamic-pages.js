...
function serviceChips(blocks = []) {
  const services = blocks.filter((block) => block.type === "to_do" && block.text).map((block) => block.text);
  return services
