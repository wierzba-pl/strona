...
const instagramProfileUrl = "https://www.instagram.com/wierzba.content/";
...
function normalizeInstagramContactLinks(html) {
  return html.replace(
    /https?:\/\/(?:www\.)?instagram\.com\/direct\/t\/EWA_WIERZBA_USERNAME\/?/gi,
    instagramProfileUrl
  );
}
...
function normalizeRegulaminToc(html) {
  if (!html.includes("tocItems")) return html;

  const sections = [...html.matchAll(/<section\b(?=[^>]*\bid=(["'])par-(\d+)\1)[\s\S]*?<h2\b[^>]*>([\s\S]*?)<\/h2>/gi)]
    .map((match) => ({
      num: match[2],
  ...
  const toc = sections
    .map(({ num, title }) => `<li>
            <a href="#par-${num}" data-target="par-${num}" style="display:flex;gap:10px;align-items:baseline;padding:7px 4px;border-radius:3px;font-size:13.5px;line-height:1.35;color:#544F45;">
              <span style="font-family:'IBM Plex Mono',monospace;font-size:12px;color:#8D8778;flex:0 0 auto;min-width:26px;">§${num}</span>
              <span style="font-family:'Source Serif 4',serif;">${title}</span>
            </a>
          </li>`)
    .join("\n");

  return html.replace(/<sc-for\b[\s\S]*?<\/sc-for>/i, toc);
}

function markMobileMediaTriptychs(html) {
  const gridPattern =
    /<div\b([^>]*\bstyle=(["'])(?=[^"']*display\s*:\s*grid)(?=[^"']*grid-template-columns\s*:\s*repeat\((?:3|5),\s*1fr\))[^"']*\2[^>]*)>/gi;

  return html.replace(gridPattern, (match, attrs, _quote, offset, source) => {
    if (match.includes("data-ew-triptych")) return match;

    const nearby = source.slice(offset, offset + 2600);
    const verticalMediaCount = (nearby.match(/aspect-ratio\s*:\s*9\/16/gi) || []).length;
    const hasProjectLinks = /Zobacz\s+wi/i.test(nearby);

    if (verticalMediaCount < 3 || !hasProjectLinks) return match;

    return `<div data-ew-triptych="true"${attrs}>`;
  });
}
...
function normalizeVideoTags(html) {
  ...
  next = next.replace(/<video\b([^>]*)>/gi, (_match, attrs) => {
    ...
    const cleaned = attrs
      ...
      .replace(/\spreload=(["'])[\s\S]*?\1/gi, "")
      .replace(/\splaysinline\b(?:=(["'])[\s\S]*?\1)?/gi, "")
      .replace(/\swebkit-playsinline\b(?:=(["'])[\s\S]*?\1)?/gi, "");

    const lazySource = sourceUrl ? ` data-src="${sourceUrl}"` : "";
    return `<video${cleaned} data-ew-lazy-video="true"${lazySource} preload="none" muted playsinline webkit-playsinline controls>`;
  });
...
function ewLoadVideo(video) {
  ewPrepareVideo(video);
  if (video.dataset.ewVideoLoaded !== "true") {
    ...
    video.dataset.ewVideoLoaded = "true";
...
function ewPrepareAllVideos() {
  ...
  videos.forEach(ewPrepareVideo);

  var initiallyVisible = videos
  ...
  initiallyVisible.forEach(function (video) {
    video.dataset.ewInitialAllowed = "true";
    ewLoadVideo(video);
  });

  if (!("IntersectionObserver" in window)) {
    videos.slice(0, 3).forEach(ewLoadVideo);
    return;
  }

  var initialGuard = true;
    ...
    function (entries) {
      entries.forEach(function (entry) {
        ...
        if (entry.isIntersecting) {
          if (!initialGuard || video.dataset.ewInitialAllowed === "true") {
            ewLoadVideo(video);
          }
        } else if (video.dataset.ewVideoLoaded === "true") {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var timer = setInterval(function () {
    document.querySelectorAll("video").forEach(ewPrepareVideo);
    tries += 1;
...
@media (max-width: 900px) {
  ...
  [data-ew-triptych="true"] {
    display: grid !important;
    grid-template-columns: 0.82fr 1.08fr 0.82fr !important;
    gap: 14px !important;
  ...
  [data-ew-triptych="true"] > *:nth-child(2) {
    transform: none !important;
  }
  [data-ew-triptych="true"] img,
  [data-ew-triptych="true"] video {
    width: 100% !important;
    height: 100% !important;
    object-fit: cover !important;
  }
  [data-ew-triptych="true"] > * > [style*="aspect-ratio"] {
    height: auto !important;
    margin: 0 auto !important;
  }
  [data-ew-triptych="true"] > *:first-child > [style*="aspect-ratio"],
  [data-ew-triptych="true"] > *:last-child > [style*="aspect-ratio"] {
    width: 82% !important;
  }
  [data-ew-triptych="true"] > *:nth-child(2) > [style*="aspect-ratio"] {
    width: 100% !important;
  }
  [data-ew-triptych="true"] > * > div:not(:first-child),
  [data-ew-triptych="true"] a {
    max-width: 100% !important;
...
export function getReferenceHtml(basePath, relativeFile) {
  ...
  html = markMobileMediaTriptychs(normalizeInstagramContactLinks(normalizeRegulaminToc(html)));
