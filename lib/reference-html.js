...
function standardTopNav() {
  ...
  <div style="max-width:1180px;margin:0 auto;padding:16px 24px;display:flex;align-items:center;justify-content:space-between;gap:20px;flex-wrap:wrap;">
    ...
    <div style="display:flex;gap:20px;flex-wrap:wrap;font-size:12.5px;text-transform:uppercase;letter-spacing:0.04em;font-weight:600;">
      ${siteNavAnchors()}
    </div>
  </div>
</nav>`;
...
function injectTopNavLinks(html) {
  const nextHtml = html.replace(/<nav\b[\s\S]*?<\/nav>/gi, (nav) => {
    ...
    const alreadyFull = siteNavLinks.every(([, href]) => next.includes(`href="${href}"`));
    if (alreadyFull) return next;
...
export function decorateHtml(html) {
  return normalizeVideoTags(injectTopNavLinks(html));
}
