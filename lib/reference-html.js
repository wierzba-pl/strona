...
const htmlRoutes = {
  ...
  "free-ukraine.html": "/free-ukraine",
...
function injectFavicons(html) {
  if (html.includes('data-ew-favicon')) return html;

  const tags = `<link data-ew-favicon rel="icon" href="/icon.png" type="image/png">
...
function injectMetaPixel(html) {
  if (html.includes("1398978959079731") || html.includes("data-meta-pixel")) return html;

  const pixel = `<!-- Meta Pixel Code -->
<script data-meta-pixel>
...
fbq('init', '1398978959079731');
fbq('track', 'PageView');
</script>
<noscript><img height="1" width="1" style="display:none"
src="https://www.facebook.com/tr?id=1398978959079731&ev=PageView&noscript=1"
alt=""></noscript>
<!-- End Meta Pixel Code -->`;

  return html.replace(/<\/head>/i, `${pixel}\n</head>`);
}

export function decorateHtml(html) {
  return injectMetaPixel(injectFavicons(injectResponsiveCss(normalizeVideoTags(injectTopNavLinks(html)))));
}
